...
# 1. DATA LOADING
file_path = 'WIND_FARM_A_compact_balanced.csv' 
df = pd.read_csv(file_path)

# 2. SAMPLING - 10000 Normal, 10000 Failure
# Seperating groups for sampling
df_normal = df[df['failure'] == 0]
df_failure = df[df['failure'] == 1]

# Select randomly 10000 rows from each
df_normal_sampled = df_normal.sample(n=10000, random_state=42)
df_failure_sampled = df_failure.sample(n=10000, random_state=42)

# Combine sampled dataframes and shuffle
df_sampled = pd.concat([df_normal_sampled, df_failure_sampled])
df_sampled = df_sampled.sample(frac=1, random_state=42).reset_index(drop=True)

# 3. FEATURE ENGINEERING 
# Voltage calculation
df_sampled['voltage'] = (df_sampled['sensor_32_avg'] + 
                         df_sampled['sensor_33_avg'] + 
                         df_sampled['sensor_34_avg']) / 3
...
df_final = df_sampled[selected_features + [target]].copy()

# 4. SAVE SAMPLED DATA FOR REFERENCE   
df_final.to_csv('sampled_wind_data.csv', index=False)
print("Örneklenmiş veri 'sampled_wind_data.csv' olarak kaydedildi.")
...
if LGBMClassifier is not None:
    lgb_pipe_tune = Pipeline([('scaler', MinMaxScaler()), ('lgb', LGBMClassifier(random_state=42))])
    lgb_param_dist = {
...
for name, mdl in models_to_plot.items():
    ...
    # Formula: (Acc × F1 × Recall × Precision × ROC-AUC) / (Log Error × Brier Error × TNR × FNR)
    # ==========================================

    tn,fp,fn,tp = confusion_matrix(y_test, y_pred).ravel()

    # TNR (True Negative Rate / Specificity) ve FNR (False Negative Rate) hesaplaması
    tnr = tn / (tn + fp) if (tn + fp) > 0 else epsilon
    fnr = fn / (fn + tp) if (fn + tp) > 0 else epsilon

    # Numerator: Acc × F1-Score × Recall × Precision × ROC-AUC
    numerator = acc * f1s * rec * prec * auc_val
    if numerator == 0:
        numerator = epsilon

    # Denominator: Log Error × Brier Error × TNR × FNR
    denominator = loss_val * brier_val * tnr * fnr
    if denominator == 0:
        denominator = epsilon

    # Overall Performance (raw)
    overall_performance_raw = numerator / denominator
...
if df_model_comp.empty:
    ...
    cols_to_pct = ['accuracy', 'precision', 'recall', 'f1_score', 'log_loss', 'brier_error', 'roc_auc', 'true_negative_rate', 'false_negative_rate']
    ...
    df_excel = df_excel[['Model', 'Acc.', 'Prec.', 'Rec.', 'F1-Sc.', 'Log Loss', 'Bri. Err.', 'ROC-AUC', 'TNR', 'FNR', 'OP']]
...
#         plt.figure()
#         try:
#             shap.summary_plot(sv, features=X_df, feature_names=X_df.columns, show=False)
